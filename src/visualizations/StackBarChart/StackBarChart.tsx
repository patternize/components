...
export const StackBarChart = () => {
  ...
  const yAxis = g => {
    return g.attr('transform', `translate(${margin.left}, ${margin.top})`).call(
      d3
        .axisLeft(y)
        .tickSizeInner(0)
        .tickSizeOuter(0)
  ...
  useEffect(() => {
    let yearIndex = 0;
    let yearData = data.filter(d => d.year === years[yearIndex]).sort(yearSort);
    ...
    setInterval(() => {
      let t = svg.transition().duration(transitionDuration);
      ...
      let yearData = data
        .filter(d => d.year === years[yearIndex])
        .sort(yearSort);
